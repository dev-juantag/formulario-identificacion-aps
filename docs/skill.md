...
1. **Director (Tagbot)**: El líder. Divide el problema, asigna roles y aprueba planes.
...
4. **Investigador**: Búsqueda de información, documentación y análisis de mercado.

5. **Revisor (Devil's Advocate)**: Busca fallos, bugs y problemas de seguridad.

## Protocolo de Orquestación Avanzada

### 1. Modo de Planificación (Gatekeeping)

Antes de realizar cambios significativos, cada agente debe enviar un **Plan de Acción** al buzón de Tagbot.

- El agente se mantiene en modo `READ_ONLY` o `PLANNING` hasta que Tagbot responda con un mensaje de `APPROVED`.

### 2. Mensajería y Difusión (Broadcast)

- **Mensaje Directo**: Coordinación 1 a 1 entre especialistas.

- **Broadcast**: Tagbot puede escribir en `broadcast.msg` para dar nuevas directrices a todo el equipo simultáneamente.
...
- Al completar una tarea, el agente debe liberar sus "locks" y notificar a Tagbot.
...
1. **Activa el Líder**: Pídele a Antigravity: *"Usa la habilidad Equipo Tagbot para inicializar este proyecto"*.

2. **Reparte el trabajo**: **  Tagbot ** dividirá el trabajo. Abre terminales nuevas para cada agente (Frontend, Marketer, etc.).

3. **Flujo de Planificación**: Los agentes envían sus planes a Tagbot antes de empezar. Un equipo bien coordinado es imparable.
